...
raw_data = raw_data[::skipper]
raw_data_size = raw_data.shape[0]
plt.plot(raw_data[::10], linewidth=0.2)
plt.show()


filter_len = 20 # filter 12, stepper 2 skipper 5 NEJ
...
for idx, sample in np.ndenumerate(raw_data):
    if counter >= 6 * filter_len:
        ...
        for i in range(filter_len):
            ...
            filter_data[counter2, i] = raw_data[idx[0] - 1 - i * stepper]
        counter2 = counter2 + 1
    counter = counter + 1
...
np.savetxt('bonn' + data_set_letter + str(stepper) + 'totalhpp.txt', totalhpp1)
